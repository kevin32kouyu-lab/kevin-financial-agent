...
export function useResearchConsole(defaultMode: RunMode = "agent") {
  ...
  // Helper to sync state between hooks
  const setAsOfDate = runManagement.setAsOfDate;
  const setHistoricalBacktestEndDate = backtestManagement.setHistoricalBacktestEndDate;
  const setReferenceStartDate = backtestManagement.setReferenceStartDate;

  const deferredSearch = useDeferredValue(runHistory.filters.search);

  // Sync terminal mode between hooks
  useEffect(() => {
    setTerminalMode(runManagementari.terminalMode);
    setAsOfDate(runManagement.asOfDate);
    setHistoricalBacktestEndDate(backtestManagement.historicalBacktestEndDate);
    ...
    if (
      ...
      runForms.setErrorText(
        locale === "zh"
          ...
          : "A run is already in progress. Wait for completion or canceling current run first.",
      );
      return;
    }
