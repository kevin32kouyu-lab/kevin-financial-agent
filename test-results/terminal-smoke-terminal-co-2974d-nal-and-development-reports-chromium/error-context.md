...
- Name: terminal-smoke.spec.ts >> terminal conclusion page switches between simple professional and development reports
- Location: web\e2e\terminal-smoke.spec.ts:601:1
...
Locator: getByRole('tab', { name: 'Simple Report' })
Expected: "true"
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('tab', { name: 'Simple Report' })
...
> 606 |   await expect(page.getByRole("tab", { name: "Simple Report" })).toHaveAttribute("aria-selected", "true");
  ...
  693 | 
  694 |   await page.goto("/terminal/archive?run=demo-run");
  695 |   await page.getByRole("button", { name: "Continue follow-up" }).first().click();
  696 | 
  697 |   await expect(page).toHaveURL(/\/terminal$/);
  698 |   await expect(page.getByRole("textbox", { name: "Investment request" })).toHaveValue(/continue monitoring/i);
  699 | });
  700 | 
```