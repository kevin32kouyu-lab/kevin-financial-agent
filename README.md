# GenAI based Agentic Financial Analyzer and Investment Advisor

GenAI based Agentic Financial Analyzer and Investment Advisor 是一个面向美股研究场景的双语投研 Agent。它把用户的自然语言问题，转成可追踪的研究流程：理解需求、筛选股票、收集数据、检索证据、生成简单版投资报告、专业版投资报告和开发者报告、校验结论，并支持回测和真实 PDF 导出。
...
- 支持终端顶部“个人投研档案中心”：登录状态、账户邮箱、记忆同步、长期偏好编辑和清空入口都在前台可见
- 支持按浏览器隔离的长期记忆：首次打开自动生成本地 `client_id`，不同浏览器彼此分开
- 支持账户版长期记忆：登录后偏好优先写入账户档案，并可把当前浏览器记忆绑定到账户
- 支持长期偏好混合学习：普通偏好可自动学习，资金规模、风险偏好、投资目标等关键偏好需要用户确认后才写入账户档案
- 支持少重复追问：后续自然语言研究会优先复用已保存的长期偏好，只补空缺，不覆盖本次明确输入；如果本次问题太模糊，会先追问而不是直接生成投资建议
- 支持结果页“本次记忆使用说明”：说明这次用了哪些长期偏好、哪些来自当前输入、哪些信息缺失所以没有自动假设
...
- 支持首次进入终端的成熟产品引导：欢迎弹窗先说明系统能做什么，再用分步浮层指向开始研究、进度、结论、回测、历史和账户入口
...
- 顶部账户入口升级为个人投研档案中心，支持查看账户邮箱、长期偏好、待确认关键偏好、浏览器记忆同步和清空账户偏好
...
- 长期记忆在账户档案中心集中展示和编辑，结果页会显示本次记忆使用说明
...
- `GET /api/v1/profile/preferences`：读取当前浏览器或当前账户的长期偏好
- `PATCH /api/v1/profile/preferences`：手动更新当前浏览器或当前账户的长期偏好，并确认关键偏好
- `DELETE /api/v1/profile/preferences`：清空当前浏览器或当前账户的长期偏好
...
- 未登录时后端按浏览器标识隔离长期记忆，登录后优先使用账户档案
- 浏览器记忆同步到账户必须由用户主动点击触发
...
- 浏览器级长期记忆（自动学习、后端保存、模糊输入不直接补全）
- 个人投研档案中心（账户偏好展示、编辑、同步、清空和关键偏好确认）
