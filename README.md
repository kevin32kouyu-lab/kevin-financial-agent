...
Financial Agent 是一个面向美股研究场景的双语投研 Agent。它把用户的自然语言问题，转成可追踪的研究流程：理解需求、筛选股票、收集数据、检索证据、生成投资报告和开发报告、校验结论，并支持回测和真实 PDF 导出。
...
- 核心卖点：可控多智能体流程、本地知识库 RAG、结论一致性校验、双报告输出、真实 PDF 报告导出、回测 V2、长期记忆。
...
- 支持正式研究报告、图表摘要与报告导出，其中投资 PDF 采用更短的平衡版模板，并由后端 Playwright/Chromium 生成真实 `.pdf` 文件
...
- 第一页新增“四步预览”，在点击前就告诉用户系统会筛选、取数、校验并生成双报告
- 高级设置默认折叠（实时 / 历史模式、日期、max results、allocation mode、live data、自定义仓位）
- 首次进入终端会先看到简短欢迎弹窗，点击开始后用分步浮层说明各功能区作用；完成或跳过后不再自动弹出
...
- 投资 PDF 默认保留 3 张关键图：推荐仓位、候选评分，以及“组合 vs 基准回测 / 风险贡献”二选一
